    ...
    <div className='md:px-[7%] py-[2%] px-4'>
      <div className='bg-white p-3 rounded-sm border'>
        <div className='flex justify-between items-center px-3 py-2'>
          <p className='md:text-3xl text-lg  font-semibold text-neutral-700'>Today's Best Deal</p>
          <Link href={'#'} className='md:text-lg text-xs font-medium text-blue-500'> See more</Link>
        </div>
        <div className='grid md:grid-cols-4 grid-cols-2 gap-4'>
          {categories.map((category) => (
            <button key={category._id} className='text-center flex flex-col items-center rounded-md '>
              <img src='src/category/tv.jpg' alt={category.name} className="zoomable-image  object-cover mb-2 rounded-md" />
